...
describes what users get. Pick a format with `-o`, or let tiengtrungbotui choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
tiengtrungbotui <command> -o table   # aligned columns for reading
tiengtrungbotui <command> -o jsonl   # one JSON object per line, for piping
tiengtrungbotui <command> -o json    # a single JSON array
tiengtrungbotui <command> -o csv     # spreadsheet friendly
tiengtrungbotui <command> -o tsv     # tab-separated
tiengtrungbotui <command> -o url     # just the URL column
tiengtrungbotui <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
tiengtrungbotui <command> --fields id,title,url
```
...
```bash
tiengtrungbotui <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
tiengtrungbotui <command>            # a table, because this is a terminal
tiengtrungbotui <command> | wc -l    # JSONL, because this is a pipe
```
